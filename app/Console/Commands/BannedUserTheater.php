...
class BannedUserTheater extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:banned-user-theater';
     ...
     * @var string
     */
    protected $description = 'Insert to table Banned_User after 1 hour not send photo for book theatre';
    ...
    public function handle()
    {
        ...
        $users = BookRoom::join('rooms', 'book_rooms.room_id', '=', 'rooms.room_id')
            ...
            ->where('rooms.name', 'like', 'Theatre%')
            ->pluck('book_rooms.NIP');

        foreach ($users as $user) {
            $bannedUser = new BannedUser();
            $bannedUser->NIP = $user;
            $bannedUser->type = 'theater';
